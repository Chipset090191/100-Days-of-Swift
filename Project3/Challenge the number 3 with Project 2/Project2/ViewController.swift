...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        countries += ["estonia", "france", "germany", "ireland", "italy", "monaco", "nigeria", "poland", "russia", "spain", "uk", "us"]
                ...
                //MARK: Challenge 3.1 from the list of Project 3 challenges
                // we need to create a custom UI Button with Info mark
                let customButton = UIButton(type: .system)
                    customButton.setTitle("ℹ️", for: .normal)
                    customButton.setTitleColor(.systemBlue, for: .normal)
                    customButton.addTarget(self, action: #selector(showScore), for: .touchUpInside)
    ...
    // MARK: Challenge 3.2 from the list of Project 3 challenges
    @objc func showScore() {
        let InfoVC = UIAlertController(title: "Info", message: "Your current score is: \(score)", preferredStyle: .alert)
        InfoVC.addAction(UIAlertAction(title: "Continue", style: .default))
