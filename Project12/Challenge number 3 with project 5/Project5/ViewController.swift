...
class ViewController: UITableViewController {
    ...
    func startGame() {
        ...
        if let existingArray = UserDefaults.standard.array(forKey: "usedWords") as? [String] {
            usedWords = existingArray
            var temp = [String]()
            temp = allWords.filter {!$0.contains(usedWords.first!)}
            title = temp.randomElement()
        }else {
...
//                    tableView.insertRows(at: [indexPath], with: .automatic)     // easier way to add some data not reloading the View itself
                    // MARK: Challenge 3.2
                    if arrayUserDefaults().contains(title!) != true {
                        usedWords.append(answer)
                        saveToUserDefaults(word: title!)
                        saveToUserDefaults(word: answer)
                    } else {
                        usedWords.append(answer)
                        saveToUserDefaults(word: answer)
                    }
    ...
    // MARK: Challenge 3.3
    func saveToUserDefaults(word: String) {
        
        if var existingArray = UserDefaults.standard.array(forKey: "usedWords") as? [String] {
            existingArray.append(word)
            UserDefaults.standard.set(existingArray, forKey: "usedWords")
        } else {
            var newArray = [String]()
            newArray.append(word)
            UserDefaults.standard.set(newArray, forKey: "usedWords")
        }
        
        }
    // MARK: Challenge 3.4
    func arrayUserDefaults() -> [String] {
        if let existingArray = UserDefaults.standard.array(forKey: "usedWords") as? [String] {
