...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //MARK: Challenge 3.2
        wind = Double.random(in: -2.0...2.0).rounded()
        physicsWorld.gravity = CGVector(dx: wind, dy: -9.8)
        
        
        let windInfo = SKLabelNode(fontNamed: "Optima")
        windInfo.fontSize = 17
        
        switch wind {
        case -2.0 ... -0.1:
            windInfo.text = "Wind: <--💨\(abs(wind * 10)) m/c"
        case 0.1 ... 2.0:
            windInfo.text = "Wind: 💨--> \(abs(wind * 10)) m/c"
        default:
            windInfo.text = "No Wind: 💨 \(abs(wind * 10)) m/c"
        }
        
        windInfo.position = CGPoint(x: 920, y: 680)
        windInfo.zPosition = 1
        addChild(windInfo)
    }
    ...
    func destroy(player: SKSpriteNode) {
        guard let looser = player.name else { return }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
            ...
            if self?.viewController?.score1 == 3 || self?.viewController?.score2 == 3 {
                ...
                //            self.changeScore(for: scoreForPlayer)
                self?.changePlayer()
    ...
    //MARK: Challenge 1.4
    func changeScore(for looser:String) {
        viewController?.Score(looser)
    }
